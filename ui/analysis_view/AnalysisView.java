...
public class AnalysisView {
    ...
    public AnalysisView(DataController data) {
        ...
        updateDebouncer = new PauseTransition(Duration.millis(150)); // Aumentato leggermente
    ...
    private void performRenderBodyAsync() {
        ...
        CompletableFuture.supplyAsync(() -> {
            ...
            // Costruiamo coach e setup (che ora gestiscono il loro loading interno)
            Node coach = coachPane.build(targetLap);
            Node setup = setupPane.build(targetLap, data.getLaps());

            body.getChildren().setAll(row, coach, setup);
