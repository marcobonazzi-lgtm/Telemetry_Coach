...
final class LapCoachPane {
    ...
    Node build(Lap lap){
        ...
        filters.setDisable(true); // Disabilitato finché non carica

        ListView<String> notesList = new ListView<>();
        notesList.getSelectionModel().setSelectionMode(SelectionMode.MULTIPLE);
        notesList.setPlaceholder(new ProgressIndicator()); // Mostra spinner mentre carica
        ...
        // Stato TTS
        ttsStatus  = new Label("In attesa analisi...");
        ...
        // Controlli (disabilitati inizialmente)
        ...
        CompletableFuture.supplyAsync(() -> {
            // Calcolo pesante in background
            return org.simulator.coach.Coach.generateNotes(lap);
        }).thenAcceptAsync(allNotes -> {
            // UI Update
            Runnable rebuild = () -> {
                ...
                ttsStatus.setText("Analisi completata");
            };
