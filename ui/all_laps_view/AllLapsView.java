...
import org.simulator.ui.analysis_view.EngineerChatPane; // Importa la nuova UI
...
/**
 * Pagina "Tutti i giri".
 * OTTIMIZZAZIONE: Rendering asincrono + Loading Overlay.
 */
public class AllLapsView {
    ...
    // --- NUOVO ROOT: StackPane per sovrapposizione ---
    ...
    public AllLapsView(DataController data) {
        ...
        // --- CREAZIONE LOADING OVERLAY ---
        loadingOverlay = new VBox(15, new ProgressIndicator(), new Label("Analisi sessione completa..."));
        loadingOverlay.setMaxSize(Double.MAX_VALUE, Double.MAX_VALUE); // Occupa tutto lo spazio
        loadingOverlay.setFillWidth(true); // Espansione orizzontale forzata
        ((Label) loadingOverlay.getChildren().get(1)).setStyle("-fx-text-fill: white; -fx-font-size: 14px; -fx-font-weight: bold;");
        ((ProgressIndicator) loadingOverlay.getChildren().get(0)).setStyle("-fx-progress-color: #0096c9;"); // Colore custom
        loadingOverlay.setAlignment(Pos.CENTER);
        loadingOverlay.setVisible(false);
        loadingOverlay.setStyle("-fx-background-color: rgba(20, 24, 30, 0.85); -fx-background-radius: 0;"); // Sfondo scuro semi-trasparente
    ...
    public GridPane grid() { return grid; } // Retrocompatibilità
    ...
    public void render() {
        ...
        Platform.runLater(() -> {
            ...
            try {
                ...
                // Ignore errors here to ensure loading screen goes away
            }
        });
    }
    ...
    private List<VBox> preparePlaceholders(List<Lap> laps) {
        ...
        for (Lap lap : laps) {
            ...
            // Placeholder vuoto (tanto c'è l'overlay sopra)
    ...
    private void renderLapAsync(int index, List<Lap> laps, AxisChoice axis, long token, List<VBox> containers) {
        ...
        // Renderizziamo a blocchi di 2 per velocizzare un po' senza freezare
        int batchSize = 2;
    ...
    private VBox buildSessionSection(List<Lap> laps) {
        ...
        // Sostituisce SessionCoachPane e SessionSetupPaneBuilder
        EngineerChatPane engineerUI = new EngineerChatPane();

        // >>> MODIFICA QUI: Attiva modalità SESSIONE <<<
