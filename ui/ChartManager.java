...
public class ChartManager {
    ...
    public void renderChart(LineChart<Number,Number> chart,
        ...
        // OTTIMIZZAZIONE: Usiamo i MinMax calcolati durante l'aggiunta delle serie
        // per impostare i limiti direttamente, senza dover ri-iterare su tutti i dati.
        if (mm.valid()){
    ...
    private void addSeries(List<XYChart.Series<Number,Number>> col,
        ...
        // OTTIMIZZAZIONE: Batch insert in lista buffer
        ArrayList<XYChart.Data<Number,Number>> buffer = new ArrayList<>(n);
        for (int i=0;i<n;i++) {
            Double xv = x.get(i), yv = y.get(i);
            if (xv == null || yv == null || xv.isNaN() || yv.isNaN()) continue;
            buffer.add(new XYChart.Data<>(xv, yv));

            // Il calcolo min/max è economico, lo facciamo qui per risparmiare tempo dopo
            if (mm != null && !xv.isInfinite() && !yv.isInfinite()) {
                if (xv < mm.xMin) mm.xMin = xv; if (xv > mm.xMax) mm.xMax = xv;
                if (yv < mm.yMin) mm.yMin = yv; if (yv > mm.yMax) mm.yMax = yv;
            }
        }
        s.getData().setAll(buffer);
        col.add(s);
    }
