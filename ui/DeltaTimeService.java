...
class DeltaTimeService {
    ...
    private static List<Double> resample(List<Double> x, List<Double> y, List<Double> xq) {
        ...
        // OTTIMIZZAZIONE: Controllo rapido se è già ordinato
        boolean sorted = true;
        for(int i=0; i < n-1; i++) {
            Double cur = x.get(i);
            Double nxt = x.get(i+1);
        ...
        if (sorted) {
            // Copia veloce senza sort
            for (int i = 0; i < n; i++) { xs[i] = x.get(i); ys[i] = y.get(i); }
        } else {
            // Sort classico lento (fallback)
