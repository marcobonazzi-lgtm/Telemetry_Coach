...
public final class ChartInteractions {
    ...
    private static final double ZOOM_IN_FACTOR  = 0.85;
    private static final double ZOOM_OUT_FACTOR = 1.15;
    private static final double PAD_RATIO = 0.0;
    ...
    private static void openFullscreen(LineChart<Number,Number> src, String title){
        ...
        // --- FIX 1: COPIA LE PROPRIETÀ (Importante per il riconoscimento del tipo di grafico) ---
        copy.getProperties().putAll(src.getProperties());
        ...
        // --- FIX 2: REGISTRAZIONE IN ChartStyles (Per i colori delle linee) ---
        ChartStyles.register(copy);
        ...
        // --- FIX 3: APPLICAZIONE TEMA CSS ---
        UiSettings s = UiSettings.get();
        ThemeManager.applyTheme(scene, s.getCurrentTheme());

        // Listener: se cambio tema nella finestra principale, cambia anche qui
