...
public final class IntroVideoView {
    ...
    public IntroVideoView(Stage stage, String resourcePath, Runnable onFinish) {
        ...
        // Nascondi "Esporta" quando l'intro è visibile
        root.sceneProperty().addListener((o, oldSc, newSc) -> {
            if (newSc != null) Platform.runLater(() -> hideEsportaMenu(newSc));
        });

        // se la Scene è già presente
        if (root.getScene() != null) {
            Platform.runLater(() -> hideEsportaMenu(root.getScene()));
        }
        // ripristina quando l'intro sparisce
        root.visibleProperty().addListener((o, ov, nv) -> { if (!nv) Platform.runLater(this::restoreEsportaMenu); });
        root.parentProperty().addListener((o, ov, nv) -> { if (ov != null && nv == null) Platform.runLater(this::restoreEsportaMenu); });
    ...
    private void hideEsportaMenu(Scene sc) {
        ...
        if (esportaMenu == null) {
            for (Menu m : mb.getMenus()) {
                String t = (m.getText() == null) ? "" : m.getText().trim();
                if (t.equalsIgnoreCase("Esporta in PDF")) {
                    esportaMenu = m;
                    break;
                }
            }
        ...
        if (esportaMenu != null) {
            esportaMenu.setDisable(true);
            esportaMenu.setVisible(false); // solo nascosto, non rimosso
        }
    }

    private void restoreEsportaMenu() {
        if (esportaMenu != null) {
            esportaMenu.setDisable(false);
            esportaMenu.setVisible(true);
            esportaMenu = null; // rilascia: se torniamo all'intro lo ricercheremo
        }
    }
}
