...
public final class ChannelAliases {
    ...
    static final Set<String> HEADER_CANDIDATES = new HashSet<>(List.of(
            ...
            // --- ACC extra (bumpstop / EN_* / lap beacon / tyre tair) ---
            "bumpstopdn ride lf","bumpstopdn ride rf","bumpstopdn ride lr","bumpstopdn ride rr",
            "bumpstopup ride lf","bumpstopup ride rf","bumpstopup ride lr","bumpstopup ride rr",
            "bumpstop force lf","bumpstop force rf","bumpstop force lr","bumpstop force rr",
            "en ap","en dy","en et","en gr","en tb","en td","en tl","en tw","en w",
            "lap beacon",
            "tyre tair lf","tyre tair rf","tyre tair lr","tyre tair rr",

            // --- LMU extra (presenti nel converted) ---
            "marker","max straight speed","min corner speed","steering shaft torque",
            "eng water temp","eng oil temp","battery charge level",
    ...
    private static Map<String, Channel> buildAliasMap() {
        ...
        PUT.accept("wheel force",         Channel.FFB);
        PUT.accept("steering force",      Channel.FFB);
        PUT.accept("Steering Wheel Position", Channel.STEER_ANGLE);
        // <<< chiave >>>
        PUT.accept("self align torque fl", Channel.FFB);
        PUT.accept("self align torque fr", Channel.FFB);

        // ===== EXTRA: LMU converted (presenti nel converted ma non nel converter) =====
        PUT.accept("marker",               Channel.MARKER);
        PUT.accept("max straight speed",   Channel.MAX_STRAIGHT_SPEED);
        PUT.accept("min corner speed",     Channel.MIN_CORNER_SPEED);
        PUT.accept("steering shaft torque",Channel.STEERING_SHAFT_TORQUE);
        PUT.accept("eng water temp",       Channel.ENG_WATER_TEMP);
        PUT.accept("eng oil temp",         Channel.ENG_OIL_TEMP);
        PUT.accept("battery charge level", Channel.BATTERY_CHARGE_LEVEL);

        PUT.accept("tyre wear fl", Channel.TYRE_WEAR_FL);
        PUT.accept("tyre wear fr", Channel.TYRE_WEAR_FR);
        PUT.accept("tyre wear rl", Channel.TYRE_WEAR_RL);
        PUT.accept("tyre wear rr", Channel.TYRE_WEAR_RR);
        PUT.accept("GPS Latitude",Channel.GPS_LATITUDE);
        PUT.accept("GPS Longitude",Channel.GPS_LONGITUDE);
