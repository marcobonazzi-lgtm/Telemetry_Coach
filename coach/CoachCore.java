 ...
 * OTTIMIZZATO PER: Assetto Corsa / LMU (Fisica differenziata per veicolo).
 */
public final class CoachCore {
    ...
    // Le soglie statiche qui sotto sono ora usate solo come fallback o per veicoli sconosciuti.
    // La logica dinamica (Profiles) ha la precedenza.
    ...
    // Nuove soglie per analisi avanzata
    public static final double OVERLAP_LIMIT = 0.03; // Max 3% del giro con freno+gas

    public static final double BRAKE_OK_MIN = 150, BRAKE_OK_MAX = 650; // Range ampio, raffinato in Setup
    ...
    // ------- ANALISI FISICHE AVANZATE -------

    /** * Nuova logica "Snap Oversteer": rileva correzioni rapide di sterzo mentre l'auto è sotto carico.
     * Più accurato di guardare solo l'angolo fisso.
     */
    public static boolean suspectedOversteer(Lap lap){
        VehicleKind kind = detectVehicleKind(lap);
        double steerThr = profSteerOversteerThr(kind); // Soglia attivazione (es. 10° F1, 30° GT)
        double latGThr  = profLatGMin(kind);           // Minimo carico laterale per considerare la curva

        if (lap.samples == null || lap.samples.size() < 10) return false;

        int sus=0;
        // Finestra mobile per ridurre il rumore
        for (int i = 2; i < lap.samples.size(); i++) {
            Sample sPrev = lap.samples.get(i-2);
            Sample sCurr = lap.samples.get(i);

            double steerPrev = val(sPrev, Channel.STEER_ANGLE);
            double steerCurr = val(sCurr, Channel.STEER_ANGLE);
            // Supporto per ACC_LAT (AC) o CG_ACCEL_LATERAL (LMU/RF2)
            double latG = val(sCurr, Channel.CG_ACCEL_LATERAL);
            if (Double.isNaN(latG)) latG = val(sCurr, Channel.ACC_LAT);

            if (Double.isNaN(steerPrev) || Double.isNaN(steerCurr) || Double.isNaN(latG)) continue;

            // Logica: Se sto curvando forte (LatG alto) MA giro il volante rapidamente
            // verso il centro (o lato opposto) -> sto correggendo.
            boolean highLoad = Math.abs(latG) > latGThr;

            // Delta sterzo (velocità approssimata)
            // Se il valore assoluto scende drasticamente mentre il carico è alto -> controsterzo
            double steerDelta = Math.abs(steerCurr) - Math.abs(steerPrev);

            // Se il carico è alto, ma "addrizzo" il volante velocemente (snap)
            if (highLoad && steerDelta < -2.5 && Math.abs(steerPrev) > steerThr) {
                sus++;
            }
        }
        // Richiediamo almeno 4 eventi distinti (frames) per evitare falsi positivi su bump
        return sus > 4;
    }
    ...
    // ======================= PROFILI MEZZO-SPECIFICI (OTTIMIZZATI AC/LMU) =======================
    private static final class Profile {
        final double thrOscDps, steerRevRate, absMuch, tcMuch, trailLow, trailHigh, coastBad, exitGasDelay;
        // Nuovi parametri fisici per riconoscimento eventi
        final double steerOversteerThr; // Gradi minimi per contare un controsterzo
        final double latGMin;           // G minimi curva

        Profile(double thrOscDps, double steerRevRate, double absMuch, double tcMuch,
                double trailLow, double trailHigh, double coastBad, double exitGasDelay,
                double steerOversteerThr, double latGMin){
            this.thrOscDps=thrOscDps; this.steerRevRate=steerRevRate; this.absMuch=absMuch;
            this.tcMuch=tcMuch; this.trailLow=trailLow; this.trailHigh=trailHigh;
            this.coastBad=coastBad; this.exitGasDelay=exitGasDelay;
            this.steerOversteerThr = steerOversteerThr;
            this.latGMin = latGMin;
        }
    }
    private static final Map<VehicleKind, Profile> PROFILES = new EnumMap<>(VehicleKind.class);
    static {
        // FORMULA/HYBRID (RSS/LMU): Sterzo direttissimo (bastano 10° per controsterzare), tollerano trail braking profondo.
        PROFILES.put(VehicleKind.FORMULA_HYBRID, new Profile(50, 180, 0.05, 0.06, 0.08, 0.35, 0.15, 0.9, 10.0, 1.2));
        PROFILES.put(VehicleKind.FORMULA,        new Profile(50, 180, 0.05, 0.06, 0.08, 0.35, 0.15, 0.9, 10.0, 1.2));

        // LMP/HYPERCAR (LMU): Pesanti, molto carico, ABS/TC sofisticati (se presenti). Tollerano "dragging" freni.
        PROFILES.put(VehicleKind.LMP,            new Profile(45, 170, 0.06, 0.08, 0.07, 0.38, 0.15, 1.1, 15.0, 1.1));

        // GT3 (ACC/AC): Sterzo meno diretto, trail braking essenziale per ruotare.
        PROFILES.put(VehicleKind.GT3,            new Profile(40, 140, 0.10, 0.12, 0.10, 0.40, 0.10, 1.2, 25.0, 1.0));
        PROFILES.put(VehicleKind.GT4,            new Profile(38, 135, 0.12, 0.14, 0.09, 0.38, 0.12, 1.25, 25.0, 0.95));

        // TOURING/DTM: Più morbide.
        PROFILES.put(VehicleKind.DTM,            new Profile(42, 150, 0.09, 0.10, 0.06, 0.35, 0.12, 1.15, 20.0, 1.1));
        PROFILES.put(VehicleKind.TOURING,        new Profile(36, 125, 0.12, 0.16, 0.05, 0.34, 0.13, 1.30, 30.0, 0.9));

        // STREET: Rollano molto, sterzo molto demoltiplicato (necessari 45°+ per chiamarlo controsterzo).
        PROFILES.put(VehicleKind.STREET,         new Profile(35, 120, 0.15, 0.18, 0.04, 0.30, 0.14, 1.35, 45.0, 0.8));

        PROFILES.put(VehicleKind.RALLY,          new Profile(34, 120, 0.15, 0.18, 0.04, 0.36, 0.14, 1.35, 40.0, 0.7));
        PROFILES.put(VehicleKind.KART,           new Profile(60, 220, 0.20, 0.20, 0.03, 0.26, 0.09, 0.9, 10.0, 1.3));
    }

    // Default profile fallback
    private static final Profile DEF_PROF = new Profile(THR_OSC_DPS, STEER_REV_RATE, ABS_MUCH, TC_MUCH, TRAIL_LOW, TRAIL_HIGH, COAST_BAD, EXIT_GAS_80_DELAY_S, STEER_DEG, LATG_HIGH);

    // Getter profilati
    public static double profThrOscDps(VehicleKind k){ return PROFILES.getOrDefault(k, DEF_PROF).thrOscDps; }
    public static double profSteerRevRate(VehicleKind k){ return PROFILES.getOrDefault(k, DEF_PROF).steerRevRate; }
    public static double profAbsMuch(VehicleKind k){ return PROFILES.getOrDefault(k, DEF_PROF).absMuch; }
    public static double profTcMuch(VehicleKind k){ return PROFILES.getOrDefault(k, DEF_PROF).tcMuch; }
    public static double profTrailLow(VehicleKind k){ return PROFILES.getOrDefault(k, DEF_PROF).trailLow; }
    public static double profTrailHigh(VehicleKind k){ return PROFILES.getOrDefault(k, DEF_PROF).trailHigh; }
    public static double profCoastBad(VehicleKind k){ return PROFILES.getOrDefault(k, DEF_PROF).coastBad; }
    public static double profExitGasDelay(VehicleKind k){ return PROFILES.getOrDefault(k, DEF_PROF).exitGasDelay; }
    public static double profSteerOversteerThr(VehicleKind k){ return PROFILES.getOrDefault(k, DEF_PROF).steerOversteerThr; }
    public static double profLatGMin(VehicleKind k){ return PROFILES.getOrDefault(k, DEF_PROF).latGMin; }
}