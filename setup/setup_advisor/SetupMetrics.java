...
public final class SetupMetrics {
    ...
    // =================advanced setup metrics (edges/camber/toe/fuel/brake/susp/aids/wind) =================
    // Nota: manteniamo la classe package-private e NON modifichiamo StyleMetrics o le API pubbliche.

    public static boolean isF(double v){ return !Double.isNaN(v) && !Double.isInfinite(v); }
    ...
    static double meanChannel(Lap lap, Channel ch){
        if (lap == null || lap.samples == null) return Double.NaN;
        double sum=0; int n=0;
        for (Sample s: lap.samples){ double v=val(s,ch); if (isF(v)){ sum+=v; n++; } }
        return n>0 ? sum/n : Double.NaN;
    }

    // ---- Tyre edge deltas (inner/middle/outer - middle) ----
    public static final class EdgeDeltas {
        final double inFL, outFL, midFL;
        final double inFR, outFR, midFR;
        final double inRL, outRL, midRL;
        final double inRR, outRR, midRR;
        EdgeDeltas(double inFL,double midFL,double outFL,double inFR,double midFR,double outFR,
                   double inRL,double midRL,double outRL,double inRR,double midRR,double outRR){
            this.inFL=inFL; this.midFL=midFL; this.outFL=outFL;
            this.inFR=inFR; this.midFR=midFR; this.outFR=outFR;
            this.inRL=inRL; this.midRL=midRL; this.outRL=outRL;
            this.inRR=inRR; this.midRR=midRR; this.outRR=outRR;
        }
        double dInMidFL(){ return inFL - midFL; } double dOutMidFL(){ return outFL - midFL; }
        double dInMidFR(){ return inFR - midFR; } double dOutMidFR(){ return outFR - midFR; }
        double dInMidRL(){ return inRL - midRL; } double dOutMidRL(){ return outRL - midRL; }
        double dInMidRR(){ return inRR - midRR; } double dOutMidRR(){ return outRR - midRR; }
        public double frontInMid(){ return avg(dInMidFL(), dInMidFR()); }
        public double frontOutMid(){ return avg(dOutMidFL(), dOutMidFR()); }
        double rearInMid(){ return avg(dInMidRL(), dInMidRR()); }
        double rearOutMid(){ return avg(dOutMidRL(), dOutMidRR()); }
        static double avg(double a, double b){ return (isF(a)&&isF(b))? (a+b)/2.0 : (isF(a)?a:(isF(b)?b:Double.NaN)); }
    }

    public static EdgeDeltas edgeDeltas(Lap lap){
        double inFL = meanChannel(lap, Channel.TIRE_TEMP_INNER_FL);
        double midFL= meanChannel(lap, Channel.TIRE_TEMP_MIDDLE_FL);
        double outFL= meanChannel(lap, Channel.TIRE_TEMP_OUTER_FL);
        double inFR = meanChannel(lap, Channel.TIRE_TEMP_INNER_FR);
        double midFR= meanChannel(lap, Channel.TIRE_TEMP_MIDDLE_FR);
        double outFR= meanChannel(lap, Channel.TIRE_TEMP_OUTER_FR);
        double inRL = meanChannel(lap, Channel.TIRE_TEMP_INNER_RL);
        double midRL= meanChannel(lap, Channel.TIRE_TEMP_MIDDLE_RL);
        double outRL= meanChannel(lap, Channel.TIRE_TEMP_OUTER_RL);
        double inRR = meanChannel(lap, Channel.TIRE_TEMP_INNER_RR);
        double midRR= meanChannel(lap, Channel.TIRE_TEMP_MIDDLE_RR);
        double outRR= meanChannel(lap, Channel.TIRE_TEMP_OUTER_RR);
        return new EdgeDeltas(inFL,midFL,outFL, inFR,midFR,outFR, inRL,midRL,outRL, inRR,midRR,outRR);
    }

    // ---- Suspension travel utilisation (% of MAX) ----
    static final class SuspUtil {
        final double fl, fr, rl, rr;
        SuspUtil(double fl,double fr,double rl,double rr){ this.fl=fl; this.fr=fr; this.rl=rl; this.rr=rr; }
        double front(){ return EdgeDeltas.avg(fl, fr); }
        double rear(){ return EdgeDeltas.avg(rl, rr); }
    }
    static SuspUtil suspUtil(Lap lap){
        double fl = ratio(meanChannel(lap, Channel.SUSP_TRAVEL_FL), meanChannel(lap, Channel.MAX_SUS_TRAVEL_FL));
        double fr = ratio(meanChannel(lap, Channel.SUSP_TRAVEL_FR), meanChannel(lap, Channel.MAX_SUS_TRAVEL_FR));
        double rl = ratio(meanChannel(lap, Channel.SUSP_TRAVEL_RL), meanChannel(lap, Channel.MAX_SUS_TRAVEL_RL));
        double rr = ratio(meanChannel(lap, Channel.SUSP_TRAVEL_RR), meanChannel(lap, Channel.MAX_SUS_TRAVEL_RR));
        return new SuspUtil(fl,fr,rl,rr);
    }
    static double ratio(double a, double b){ return (isF(a)&&isF(b)&&b!=0.0) ? (a/b) : Double.NaN; }

    // ---- Fuel ----
    static final class FuelStats {
        final double level;   // avg L
        final double rate;    // L/lap (se disponibile) oppure L/s istantaneo
        final double maxFuel; // L se presente
        FuelStats(double level,double rate,double maxFuel){ this.level=level; this.rate=rate; this.maxFuel=maxFuel; }
    }
    static FuelStats fuelStats(Lap lap){
        return new FuelStats(
                meanChannel(lap, Channel.FUEL_LEVEL),
                meanChannel(lap, Channel.FUEL_RATE),
                meanChannel(lap, Channel.MAX_FUEL)
        );
    }

    // ---- Brakes ----
    static final class BrakeStats {
        final double avgFL, avgFR, avgRL, avgRR;
        final double brakeTimePct;
        BrakeStats(double avgFL,double avgFR,double avgRL,double avgRR,double brakeTimePct){
            this.avgFL=avgFL; this.avgFR=avgFR; this.avgRL=avgRL; this.avgRR=avgRR; this.brakeTimePct=brakeTimePct;
        }
    }
    static BrakeStats brakeStats(Lap lap){
        double fl = meanChannel(lap, Channel.BRAKE_TEMP_FL);
        double fr = meanChannel(lap, Channel.BRAKE_TEMP_FR);
        double rl = meanChannel(lap, Channel.BRAKE_TEMP_RL);
        double rr = meanChannel(lap, Channel.BRAKE_TEMP_RR);
        // stimiamo % tempo in frenata da BRAKE_POS > 0.05
        int n=0, nb=0;
        if (lap!=null && lap.samples!=null){
            for (Sample s: lap.samples){
                n++;
                double bpos = val(s, Channel.BRAKE_POS);
                if (isF(bpos) && bpos > 0.05) nb++;
            }
        }
        double pct = (n>0) ? ((double)nb)/n : Double.NaN;
        return new BrakeStats(fl,fr,rl,rr,pct);
    }
